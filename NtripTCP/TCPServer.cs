...
namespace NTRIPClient
{
    class TCPServer
    {
        ...
        public DateTime RecentDataTime{get;set;}
        ...
        public void StopServer() //This gets called by a button on another form, and on form close.
        {
            ...
            {
                TCPForm.Default.SendStuffToUIThread(0, 0, "TCP Server停止监听" + port + "端口");
            }
        }
        private void DoListen()
        {
            ...
            TCPForm.Default.SendStuffToUIThread(0, 0, "TCP Server正在监听" + port + "端口");
            ...
            try //The try block is merely to exit gracefully when you stop listening
            {
                do //Loop to handle multiple connections
                {
                    ...
                    if (!(socket == null)) //can't hurt to double-check
                    {
                        ...
                        {
                            TCPForm.Default.SendStuffToUIThread(0, 0, "TCP客户端已连接" + port + "端口");
                        }
                    }
                }
                while (true);
                                ...
                                if (client.Socket != null) //Do we have a socket
                                {
                                    ...
                                    if (client.Socket.Poll(10, SelectMode.SelectRead) == true) //Did we receive data
                                    {
                                        try //Try reading from the socket
                                        {
                                            ...
                                            //    TCPForm.Default.TCPUpdateUIThread(3, pointName, bytes);
                                            //}
                ...
                lock (clients) //Remove any dead sockets
                {
                    ...
                    do
                    {
                        ...
                        for (int i = 0; i <= clients.Count - 1; i++)
                        {
                            ...
                            {
                                TCPForm.Default.SendStuffToUIThread(2, ((ClientContext)(clients[i])).ConnID, null); //Remove ID from the connections table on the UI thread
                    ...
                    if (clients.Count != curCount) //Client count changed
                    {
                        if (clients.Count == 0)
                        {
                            TCPForm.Default.SendStuffToUIThread(0, 0, "TCP客户端已断开连接"+port + "端口");
                        }
                    }
                }
